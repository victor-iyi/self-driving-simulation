...
def _img_func(row: dict):
    ...
    Returns:
        dict: {DataKeys.IMAGES: tf.Tensor}
    ...
    """
    image_string = tf.decode_base64(row[DataKeys.IMAGES])
    ...
    return {
        DataKeys.IMAGES: image_cast,
        DataKeys.LABELS: row[DataKeys.LABELS]
    }
...
def _parser(row: dict):
    ...
    Returns:
        dict: {DataKeys.FILENAMES: tf.float32, DataKeys.LABEL: tf.float32}
            decoded image & reshaped label.

    """
    # Read the contents in filename as a string.
    image_string = tf.read_file(row[DataKeys.IMAGES])
    ...
    # Reshape label.
    label_reshape = tf.reshape(row[DataKeys.LABELS], shape=(1,))

    # Return parsed image & label.
    return {
        DataKeys.IMAGES: image_cast,
        DataKeys.LABELS: label_reshape
    }
